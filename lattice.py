...
class HexagonalLattice(object):
    ...
    def __init__(self, Nx, Ny):
        """Initialize class."""
        # Start with a normal OpenLattice of the same size.
        big_lat = OpenLattice(Nx, Ny)
        ix_pts = np.nonzero(np.logical_and(
            big_lat.xy_coords[:, 0] <= Nx*3/4,
            big_lat.xy_coords[:, 0] >= -Nx*3/4
            ))[0]
        self.L = np.size(ix_pts)
        self.xy_coords = big_lat.xy_coords[ix_pts]
